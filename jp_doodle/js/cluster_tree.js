    ...
    $.fn.cluster_tree = function (options, element) {
        ...
        class ClusterTree {
            constructor(options, element) {
                ...
                this.end_depth = Math.min(this.end_depth, this.max_depth);
            ...
            adjusted_y(offset) {
                var low = this.low;
                var high = this.high;
                var doffset = this.settings.max_y * (1.0 / (high - low + 1));
                return (offset - low + 0.5) * doffset;
            }
        ...
        class ClusterNode {
            ...
            pick(value) {
                // override default implementation for internal nodes.
                if (value > this.high) {
                    console.warn("picking too high: " + [this.high, value]);
                }
                if (value < this.low) {
                    console.warn("picking too low: " + [this.low, value]);
                }
                this.callback();
            }
        ...
        class InternalNode extends ClusterNode {
            ...
            pick(value) {
                if (value > this.high) {
                    ...
                    if ((this.above.high >= value) && (this.above.low <= value)) {
                        return this.above.pick(value);
                    }
                    if ((this.below.high >= value) && (this.below.low <= value)) {
                        return this.below.pick(value);
                    }
                }
                // default
                this.callback();
        ...
        class LeafNode extends ClusterNode {
            draw(in_tree, frame) {
                if (this.in_tree.settings.debug)  {
