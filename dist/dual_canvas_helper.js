...
(function ($) {
    ...
    $.fn.dual_canvas_helper = function (options) {
        ...
        target.off_canvas_event = function (event_type, for_name_or_info) {
            if (for_name_or_info) {
                var object_info = target.get_object_info(for_name_or_info);
                var for_name = object_info.name;
                if (object_info && target.event_info.object_event_handlers[event_type] && target.event_info.object_event_handlers[event_type][for_name]) {
        ...
        target.object_name_at_position = function (event, pixel_x, pixel_y) {
            ...
            var color_index = target.color_index_at(target.invisible_canvas, pixel_x, pixel_y);
        ...
        target.generic_event_handler = function (e) {
            ...
            e.canvas_name = target.object_name_at_position(e, e.pixel_location.x, e.pixel_location.y);
        ...
        target.vector_frame = function (x_vector, y_vector, xy_offset, name) {
            return target.dual_canvas_helper.reference_frame(target, x_vector, y_vector, xy_offset, name);
        };
    ...
    $.fn.dual_canvas_helper.add_geometry_logic = function (target) {
        ...
        target.color_chooser = function (config, element) {
            ...
            var intensity_frame = element.frame_region(settings.x + left + 10, settings.y + 10, settings.x + side - 20, settings.y + side - 20, 0, 0, 1.0, max_intensity + slider_height);
            ...
            var triangle_frame = element.vector_frame({ x: left, y: 0 }, { x: left * 0.5, y: top - 20 }, // Y axis is slanted to the right.
        ...
        target.axis = function (config) {
            ...
            var params = $.extend({
                ...
                ticks: [{ offset: 0, text: "0.0", name: "Zero" }, { offset: 10, text: "ten", name: "Ten" }, { offset: 15, text: "15.0", name: "fifteen" }],
            ...
            var tick_direction_length = target.vlength(target.vadd(target.converted_location(params.tick_direction.x, params.tick_direction.y), target.vscale(-1, target.converted_location(0, 0))));
            ...
            for (var i = 0; i < ticks.length; i++) {
                ...
                tick.origin = target.vadd(params.axis_origin, target.vscale(tick.offset, params.offset_vector));
    ...
    $.fn.dual_canvas_helper.reference_frame = function (parent_canvas, x_vector, y_vector, xy_offset, name) {
    ...
    $.fn.dual_canvas_helper.example = function (element, x, y, w, h) {
        ...
        var click_handler = function (e) {
            info.html("<div>click at " + e.pixel_location.x + " " + e.pixel_location.y + " " + e.canvas_name + " " + e.invisible_color + "</div>");
        };
        var mouse_over_circle_handler = function (e) {
            info.html("<div>rect mouse over at " + e.pixel_location.x + " " + e.pixel_location.y + " " + e.canvas_name + " " + e.invisible_color + "</div>");
        };
        ...
        var put_circle = function (event) {
            ...
            info.html("<div> [" + Math.round(loc.x) + ", " + Math.round(loc.y) + "] :: [" + Math.round(ploc.x) + ", " + Math.round(ploc.y) + "] </div>");
    ...
    $.fn.dual_canvas_helper.frame_example = function (element) {
        ...
        var rotate = element.vector_frame({ x: 1, y: 2 }, { x: 1, y: -1 }, { x: 240, y: 160 });
        rotate.text({
            text: "Rotate/skew",
            align: "right",
            x: 20,
        ...
        for (var i = 0; i < frames.length; i++) {
            ...
            var points = [[20, 20], [40, 20], [20, 40], [40, 40], [40, 60]];
