    ...
    $.fn.rectangle_collection = function (options) {
        ...
        target.focus_anchors = function (u_anchor, v_anchor) {
            ...
            for (var i = 0; i < bars.length; i++) {
                var bar = bars[i];
                if (bar.u_anchor == u_anchor || bar.v_anchor == v_anchor || !u_anchor && !v_anchor) {
                    // make it visible
        ...
        target.draw_bars = function (no_redraw) {
            ...
            if (labels_position) {
                ...
                var set_label = function (vec, name, text) {
                    ...
                    var move_label = function (event) {
                        ...
                        var new_vec = vscale(1.0 / labels_scale, vadd({ x: loc.x, y: loc.y }, vscale(-1, labels_position)));
                        // modify vec in place
                        $.extend(vec, new_vec);
                        // redraw
                        target.draw_bars();
            ...
            target.on_canvas_event("click", function () {
                target.focus_anchors();
            });
            if (target.bar_fit) {
            ...
            target.dialog_members = "dialog dialog_u dialog_v dialog_h".split(" ");
            ...
            var put_dialog = function (event) {
                ...
                target.set_visibilities(target.dialog_members, true);
